...
// ---------- Helpers ----------
const formatEuro = v => v.toLocaleString('fr-FR',
...
function updateTotal(){
  const durationHours = (parseFloat(durationInput.value) || 0) / 60;
  let total = 0;
...
function toggleHoursInput(row){
  ...
  hoursInput.style.display =
       row.querySelector('.salary-type').value === 'annuel' ? 'inline-block' : 'none';
}
...
function createParticipantRow(){
  ...
  row.innerHTML = `
    ...
    <input class="hours-input" type="number" min="1" max="60" value="35"
           title="Heures/sem">
    <button class="remove-btn" title="Supprimer">🗑️</button>`;
  // Événements
  row.querySelector('.salary-type')
     .addEventListener('change',()=>{toggleHoursInput(row);updateTotal();});
  row.querySelectorAll('input,select')
     .forEach(el=>el.addEventListener('input',updateTotal));
  row.querySelector('.remove-btn')
     .addEventListener('click',()=>{row.remove();updateTotal();});
